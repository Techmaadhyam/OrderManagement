    ...
    axios
      ...
      .then((response) => {
        ...
        for (const { workOrder, monthname } of response.data) {
          ...
          groupedData[month].push(workOrder); // Append the order to the respective group
        }
  ...
  const renderTablesForMonths = () => {
    ...
    return Object.entries(userData).map(([month, data]) => {
      const dataWithKeys = data?.map((item) => ({
        ...item,
        companyName:
          item?.noncompany?.companyName || item?.company?.companyName,
        gstn: item?.noncompany?.gstNumber || item?.noncompany?.gstNumber,
                    ...
                    [
                      {
                        columns: [
                          ...
                          {
                            text: `Work Details of ${month} ${year}`,
                            style: "header",
                            alignment: "center",
                          },
